...
### Specification: The Auction functionality is currently disabled through Polygon's Smart Contracts. Through the Auction Mechanism, a Validator will go through a bidding Mechanism where the newer validator has to bid higher than the current validator that is occupying the slot. Once the Auction period is over, whoever has the highest bid at the end of it becomes the validator.
...
**Auction Smart Contract**: https://github.com/maticnetwork/contracts/blob/v0.3.0-backport/contracts/staking/stakeManager/StakeManagerExtension.sol
...
1. **What happens if the Validator does not "Confirm Bid" for a long time after being declared the winner after an auction period?**
In this case, there is a cooldown period till when the validator can "Confirm Bid". Once that period is over, he loses that slot and now that slot would be available to anyone

2. **Can a new validator make multiple bids during an auction period?**
Yes, a new validator can make multiple bids to increase his chances. But this also means that the new validator will need lots of Matic tokens to bid on multiple validator slots

3. **How long would the tokens be locked for if I have lost in a Bid?**
As long as you're the highest bidder your tokens will be locked. The moment you're not the highest bidder, the tokens will be transferred back to your account

4. **Are all the auction transactions happening on Ethereum?**
Yes
